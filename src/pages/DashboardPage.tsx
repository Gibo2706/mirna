...
import type { CommitmentOccurrence, FinanceSnapshot, PlannedEvent } from '@/domain/types';
import {
...
export const DashboardPage = ({ snapshot }: { snapshot: FinanceSnapshot }) => {
  ...
  const markPaid = (item: (typeof upcoming)[number]) => {
    setActionError('');
    try {
      if (item.kind === 'commitment') {
        setPayingCommitment(item);
        return;
      } else {
  ...
  return (
    <main className="screen">
      <PageHeader
        ...
        description="Vaš novac danas i obaveze koje dolaze."
      />
      {actionError ? (
      ...
      <div className="grid min-w-0 grid-cols-1 gap-5 lg:grid-cols-[1.15fr_0.85fr]">
        <section className="section-stack">
          <Card
            className={`relative overflow-hidden rounded-2xl border-0 p-5 text-white sm:p-6 ${safeToSpend < 0 ? 'bg-[#833d3d]' : 'bg-[#17251f]'}`}
          >
            <button
            ...
            <p className="money relative mt-2 text-4xl font-extrabold tracking-[-0.03em] sm:text-5xl">
          ...
          {hasFinancialActivity ? (
            <div className="grid grid-cols-2 gap-x-5 gap-y-4 border-b py-4">
              {[
                [
                  'Stvarni prihod',
          ...
          {budgetProgress.length ? (
            <section className="finance-section">
              <div className="flex items-center justify-between gap-4">
                <div>
                  <p className="text-sm font-semibold text-muted">Promenljivi budžet</p>
              ...
              {upcoming.length ? (
                upcoming.map((item) => {
                  ...
                  return (
                    ...
                    >
                      <div className="grid h-10 w-5 shrink-0 place-items-center text-muted">
                  ...
                  .map((goal) => {
                    ...
                    return (
                      <Link
                        key={goal.id}
                        to="/goals"
                        className="min-w-0 py-4 transition hover:bg-surface-2"
                      >
              ...
              {recentTransactions.length ? (
                recentTransactions.map((transaction) => (
                  <div key={transaction.id} className="flex items-center gap-3 p-4">
                    <span
                      className={`grid w-5 shrink-0 place-items-center ${transaction.type === 'expense' ? 'bg-danger-soft text-danger' : transaction.type === 'income' ? 'bg-accent-soft text-accent' : 'bg-surface-2 text-muted'}`}
                    >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-xs text-muted">
                        ...
                        {transaction.type === 'transfer'
                          ? ` · ${snapshot.accounts.find((a) => a.id === transaction.accountId)?.name ?? ''} → ${snapshot.accounts.find((a) => a.id === transaction.toAccountId)?.name ?? ''}`
                          : ''}
                      </p>
                    </div>
                    <p
